...
// Handle /x command
export async function getUsernameHistory(ctx: Context) {
  const twitterHandle = ctx.match;
  ...
  try {
    ...
    ctx.reply("Failed to fetch mentions. Please try again later.");
  }
}

// Handle /nfl command (number of followers)
export async function getUsernameFollowerCount(ctx: Context) {
  const twitterHandle = ctx.match;
  ...
  try {
    ...
    await ctx.reply(`👥 Followers for @${twitterHandle}: ${followerCount}`);
  } catch (error) {
    console.error("Error fetching follower count:", error);
    ctx.reply("Failed to fetch follower count. Please try again later.");
  }
}

// Handle /scam command
export async function getUsernameScamStatus(ctx: Context) {
  const twitterHandle = ctx.match;
  ...
  try {
    ...
    if (isScam) {
      return ctx.reply(`⚠️ Warning: This account is associated with scams.`);
    } else {
      return ctx.reply(
...
export async function getUsernameLatestTweet(ctx: Context) {
  ...
  try {
    ...
    if (latestTweet) {
      ...
      await ctx.reply(`No tweets found for @${twitterHandle}.`);
    }
  } catch (error) {
    console.error("Error fetching tweets:", error);
    ctx.reply("Failed to fetch tweets. Please try again later.");
  }
}
...
--------------------------------------
*/
const getUserProfile = async (twitterHandle: string | RegExpMatchArray) => {
...
const getUserId = async (twitterHandle: string | RegExpMatchArray) => {
  const { data } = await getUserProfile(twitterHandle);
  console.log("twitter data is", data.id);
  return data.id;
};
